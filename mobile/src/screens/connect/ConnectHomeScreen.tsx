...
import {
    ...
    fetchHelpRequests, fetchOrCreateChat, fetchReceivedRequests, fetchStudyGroups, fetchTechReviews,
...
export default function ConnectHomeScreen() {
  ...
  async function handleAcceptRequest(item: ConnectionReqType) {
    const reqId = item._id || item.id;
    try {
      await acceptConnectionRequest(reqId);
      refreshTab('messages');
      // Navigate to chat with the accepted user
      await fetchOrCreateChat(item.senderId);
      navigation.navigate('Chat', {
        participantId: item.senderId,
        participantName: item.senderName,
      });
