    ...
    try {
      ...
      if (io) {
        await createNotification({
          ...
          message: `${userName} wants to join "${project.title}"`,
          data: { projectId: actualProjectId, requestId: request._id.toString(), userId: req.user?.id },
        }, io);
      }
    } catch (emailError) {
      ...
      if (io) {
        if (status === 'approved') {
          await createNotification({
            ...
            data: { projectId: project._id.toString(), requestId: req.params.requestId },
          }, io);
        } else if (status === 'rejected') {
          await createNotification({
            userId: request.userId.toString(),
