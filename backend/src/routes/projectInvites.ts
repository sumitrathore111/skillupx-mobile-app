...
This invite link expires in 7 days.`;
    ...
    if (io) {
      ...
      await createNotification({
        ...
        message: `${inviterName} invited you to join "${project.title}"`,
        data: { projectId, inviteId: invite._id.toString(), inviteLink },
      }, io);
    }
      ...
      if (io) {
        ...
        if (invite.invitedBy) {
          await createNotification({
            ...
            message: `${user.name} joined "${project.title}"`,
            data: { projectId: project._id.toString(), userId: user._id.toString() },
          }, io);
        }
      }
    }
